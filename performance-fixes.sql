...
ON public.generated_posts(brief_id, status, created_at DESC);

-- Index for real-time subscription filtering
CREATE INDEX IF NOT EXISTS idx_content_briefs_user_status_updated 
ON public.content_briefs(user_id, status, updated_at DESC);
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    PERFORM pg_notify(
      ...
      json_build_object(
        ...
        'operation', TG_OP,
        'id', NEW.id,
        'user_id', COALESCE(NEW.user_id, NEW.brief_id),
        'status', NEW.status
      )::text
    );
