...
export default function EnhancedAnalyticsDashboard({
  ...
  const engagementData = useMemo(() => {
    ...
    const groupedData = analytics.reduce(
      (
        acc: Record<
          string,
          {
        ...
        const platform = item.platform as
          | "linkedin"
          | "twitter"
          | "facebook"
          | "instagram";
  ...
  const aiPerformanceData = useMemo(() => {
    const openAIPosts = analytics.filter(
      (a) => a.generated_posts?.ai_provider === "openai"
    );
    const vertexPosts = analytics.filter(
      (a) => a.generated_posts?.ai_provider === "vertex"
    );

    const openAIMetrics = openAIPosts.length
      ? {
          engagement: (
            (openAIPosts.reduce((sum, p) => sum + p.engagement_rate, 0) /
              openAIPosts.length) *
            100
          ).toFixed(0),
          reach: Math.floor(
            openAIPosts.reduce((sum, p) => sum + p.views_count, 0) /
              openAIPosts.length /
              100
          ),
          conversions: (
            (openAIPosts.reduce((sum, p) => sum + p.clicks_count, 0) /
              openAIPosts.length) *
            10
          ).toFixed(0),
    ...
    const vertexMetrics = vertexPosts.length
      ? {
          engagement: (
            (vertexPosts.reduce((sum, p) => sum + p.engagement_rate, 0) /
              vertexPosts.length) *
            100
          ).toFixed(0),
          reach: Math.floor(
            vertexPosts.reduce((sum, p) => sum + p.views_count, 0) /
              vertexPosts.length /
              100
          ),
          conversions: (
            (vertexPosts.reduce((sum, p) => sum + p.clicks_count, 0) /
              vertexPosts.length) *
            10
          ).toFixed(0),
  ...
  const platformMetrics = useMemo(() => {
    ...
    return platforms.map((platform) => {
      ...
      const engagement = platformPosts.length
        ? (platformPosts.reduce((sum, p) => sum + p.engagement_rate, 0) /
  ...
  const recentPosts = useMemo(() => {
    return analytics.slice(0, 4).map((post) => ({
      id: post.id,
      content:
        post.generated_posts?.content?.substring(0, 50) + "..." ||
        "Content preview...",
      ...
      ai:
        post.generated_posts?.ai_provider === "openai" ? "OpenAI" : "Vertex AI",
    }));
  }, [analytics]);
  ...
  const overviewMetrics = useMemo(() => {
    ...
    const totalClicks = analytics.reduce(
      (sum, item) => sum + item.clicks_count,
      0
    );
    ...
    <div className={`min-h-screen bg-gray-50 p-6 ${className}`}>
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Advanced insights and performance tracking across all platforms
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <MetricCard
            title="Total Reach"
            value={overviewMetrics.totalReach}
            change={12.5}
            icon={Eye}
            color="blue"
          />
          ...
          <MetricCard
            ...
            value={overviewMetrics.clickThroughRate}
            change={-2.1}
            icon={MousePointer}
            color="green"
          />
          <MetricCard
            ...
            value={overviewMetrics.followerGrowth}
            change={15.7}
            icon={Users}
            color="purple"
          />
          ...
          <div className="overflow-x-auto">
            <div className="grid grid-cols-8 gap-2 min-w-[600px]">
              ...
              {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
                <div
                  key={day}
                  className="font-medium text-gray-600 text-center py-2"
                >
                  {day}
                </div>
              ))}
